...
// 存音乐的详情
var musicData = [['25216950228','陈稣伟'],['金陵赋','程先甲'],['洛都赋','傅毅'],['洛神赋','曹植']];
...
function initMusic(){
   ...
   audio.onloadedmetadata = function(){
    ...
    setBackground(`img/bg${musicId}.png`);
...
function updateActiveSong() {
    const songItems = document.querySelectorAll('.musiclists div');
...
playPause.addEventListener('click',function(){
    if(audio.paused){  // 检测音频当前状态
        audio.play();  // HTML5 Audio API播放方法
        rotateRecord();  // 同步启动唱片旋转
        // 切换按钮图标类名
        playPause.classList.remove('icon-play');
        playPause.classList.add('icon-pause');
    } else {
        audio.pause();  // HTML5 Audio API暂停方法
        rotateRecordStop();  // 同步停止唱片旋转
        // 切换按钮图标类名
...
function formateTime(value){
    var s = value.split(':');
    return s.map(num => num.padStart(2, '0')).join(':');
}
...
function createMusicList(){
    for(let i=0;i<musicData.length;i++) {
        ...
        div.innerText = `${musicData[i][0]} - ${musicData[i][1]}`;
